...
class News extends React.Component {
  ...
  //load
  loadList = async () => {
    const res = await axios.get('http://localhost:8080/admin/news')
    console.log(res)
    this.setState({
      list: res.data
    })
  ...
  handleDelete = async (id) => {
    const res = await axios.delete(`http://localhost:8080/admin/news/${id}`)
    this.setState({
      list: res.data
    })
    this.loadList()
    ...
    columns: [
      ...
      },
      {
        ...
        render: (text, record) => (
          <Space size="middle">
            <Button>
              <Link to={"/editNews/" + record.id}>Edit</Link>
            </Button>
          </Space>
        ),
      },
